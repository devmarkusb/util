...
{
    std::string s;
    const auto ret = too::str::apply_ellipse(s, 0);
...
TEST(str_apply_ellipse, test_german)
{
    ...
    const auto ret = too::str::apply_ellipse(s, 2);
    EXPECT_STREQ("xy...", ret.c_str());

    const auto ret2 = too::str::apply_ellipse(s, 1);
    EXPECT_STREQ("x...", ret2.c_str());

    const auto ret3 = too::str::apply_ellipse(s, 3);
    EXPECT_STREQ("xyz", ret3.c_str());

    const auto ret4 = too::str::apply_ellipse(s, 4);
