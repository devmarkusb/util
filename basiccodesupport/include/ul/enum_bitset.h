...
namespace mb::ul {
/** Wrap your enum EnumType into a type allowing logical combinations of the enum's elements by mapping the enum enum_values
    to bit positions (starting with LSB). Requirements: The enum must contain an element called `end` being the last one
    (having a enum_value greater than everyone else). The enum_value of `end` needs to be less than or equal the number of bits
    within type SizeType plus 1. The enum should not contain enum_values smaller than 0 (at least you have to take care
    yourself, that you don't do manipulations with it with this class). It is allowed to contain gaps
    though.
    Special case: the end enum_value is identified with 'empty' or 'no enum_value' or 0, which is the default initialization also.
    */
template <
    EnumConcept EnumType, std::integral SizeType = std::underlying_type_t<EnumType>, bool experimental_implicit = false>
class EnumBitset {
public:
    static_assert(enum_cast(EnumType::end) <= sizeof(SizeType) * bits::bits_per_byte + 1);
