...
#pragma message("Note that TOO_ASSERT_ALWAYS_THROWING is defined, which leads to a diagnostic mode of letting every TOO_ASSERTION throw.")
#endif
#endif
#endif
...
/** Proposal of usage guidelines (at the example of the TOO_EXPECT* macros, ASSERTs and ENSUREs are analogous).
	...
	3.	More interesting though is, when to use either of the following:
		...
		b) if (!cond) return; // in this case of course paired with a preceding TOO_EXPECT, telling about a real error case
		That's basically when you know your program would crash or show undefined behavior would it run further down
		the routine.
		Then you could choose b) if there was no harm in letting the error go unnoticed except the program 'just' missing some
		functionality.
		But in most cases you should opt for a), letting programmers and testers know that there is a logically wrong behavior
		happening. The exception too::fail_fast can then be caught at any call stack level that thinks it knows how to handle
		that.
*/
//!@{
...
//! Letting thread sleep if condition is not met. (e.g. to break into the debugger at any time wanted). Release builds are also affected.
...
#define TOO_VERIFY(cond)								do { volatile bool TOO_NONAME = (cond); TOO_NONAME; } while(false)
#endif
