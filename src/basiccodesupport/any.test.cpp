#include "ul/basiccodesupport/any.h"
#include "ul/basiccodesupport/std/std_extensions.h"
...
struct AFactory {
    static std::unique_ptr<IA> create_a();
};
...
std::unique_ptr<IA> AFactory::create_a() {
...
TEST(ul_anyTest, test) {
    ...
    std::unique_ptr<a::IA> ia = afactory.create_a();
    const ul::any a1 = 5;
    const ul::any test(5);
